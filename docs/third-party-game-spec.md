...
**Status:** v1.8 · **Last updated:** 2026-08-19
...
| `cr_book` | The core-content slug (e.g. a book) — present only for a 3-tier game (§5.1, Layout B). |
...
- One icon per tile — i.e. per (engine, language) for a 2-tier game, or per
  (engine, core, language) for a 3-tier game.
...
Every game has an **engine** tier and one or more **lang** tiers. Choose one of
two supported structures based on whether the game has a meaningful content
unit that is shared across multiple languages:

| Tier | What it is |
|---|---|
| **engine** | The main game engine / runtime (top tier, always present) |
| **core** | Optional language-agnostic mid tier: content shared across a core unit's languages, e.g. a book's illustrations and structure |
...
> `-core` filename token, and the optional **core** ZIP uses the `-book-` token.
> The filename tokens are frozen (the pipeline classifies ZIPs by them); the
> tier names are what people and the CMS UI use. The tables below show both.

Two layouts are supported. **Do not rename the engine tier to “core.”** The
engine is always the top-level runtime; its historical filename happens to use
the `-core` token. The optional middle **core** tier is a distinct layer used
only in Layout B.

Use this decision table:

| Your content model | Required structure |
|---|---|
| There is no language-independent content unit reused across languages | **engine + lang packs** (Layout A) |
| A content unit has material reused across multiple languages — for example, a book's illustrations and structure are shared while its text and audio vary by language | **engine + core + lang packs** (Layout B) |

In both cases the first tier is **engine** (sometimes abbreviated **eng**).
Never change an `eng`/engine tier into a core tier merely because its ZIP
filename is `<engine>-core.zip`. “Core” means the optional shared-content
middle tier, not the engine/runtime.
...
**Layout B — 3-tier (shared-content / book-style).** One tile per (engine,
core, language). Use it when a content unit has language-independent material
that will be reused by more than one language — for example, a book's
illustrations and interaction structure in the core ZIP, with translated text
and language-specific audio in each lang ZIP. The core slug travels on the
wire as `cr_book` / `bookSlug` (frozen physical names).

| Tier | Filename | Contents | One per |
|---|---|---|---|
| engine | `<engine>-core.zip` | Engine runtime (JS/CSS/shared assets) | engine version |
| core | `<engine>-book-<coreSlug>.zip` | Language-agnostic content (illustrations, structure, other shared assets) reused across that unit's languages | core unit (e.g. book) |
...
- The tokens `-core` (engine tier), `-lang-<langCode>` (lang tier), and, for
  Layout B, `-book-<coreSlug>` (core tier), are
  **structural** — the container's tooling classifies ZIPs by these patterns.
  Do not deviate.
- In Layout B, `<coreSlug>` identifies the core-tier
  content unit (e.g. the book slug): stable, lowercase, no spaces.
...
- **One ZIP per language** (or, for Layout B, per (core, language)). Never
  combine multiple languages in one ZIP.
...
- In Layout B, the core ZIP is extracted once per (engine, core unit).
...
For Layout B, the core ZIP carries the content unit's
language-agnostic subtree, and the lang ZIP carries only that unit's
`lang/<langCode>/` (or equivalent per-language) files — same merge-cleanly
rule.
...
- Layout B: `https://yourplayer.example.org/?cr_book={book}&cr_lang={lang}`

Only the **query part** matters for the offline container — `{lang}` (and,
for Layout B, `{book}`, the core-unit slug) is substituted per
tile, and the resulting query is appended to the local
`file://…/index.html`. (The host part is used only when the game is served
remotely.) The container additionally appends `cr_user_id`. Your game must
parse all applicable parameters from `window.location.search`.
...
- [ ] Each lang ZIP contains only `lang/<langCode>/…` (or, for Layout B, its per-(core, language) subtree)
...
a separate **promotion** step makes it the current version in the CMS, and a
final **staff Publish** step (Curious Learning-only, §8.5a) is what actually
ships content to production devices.
...
- Auth: a **personal MCP token**, sent as `Authorization: Bearer <token>`
  **or** `x-api-key: <token>`.

**Getting a token.** Tokens are personal, not shared: any signed-in CMS user
creates one on the admin site's **"MCP tokens"** tab (name it after your team
or bot). The token value (`crmcp_…`) is displayed **exactly once** at creation
with a copy button — store it in a secret manager immediately; afterwards the
CMS shows only the name, permission, creation date and last-4 characters, and
the value can never be viewed again. A lost token is simply deleted and
replaced. Deleting a token revokes its MCP access immediately.

**Permission scopes.** Each token carries one of two scopes, chosen at
creation (what a user may grant is capped by their CMS role):
...
error naming the required scope. Everything done with a token is attributed
to the CMS user who created it.
...
| `promote_content` | Move an item to production (demotes the prior production version) |
| `demote_content` | Move a production item back to development; supports `dryRun` preview and dependency `cascade` |
| `update_version_notes` | Edit the free-text version notes on an already-uploaded item (`{ itemId, versionNotes }`) |
| `list_manifests` | List the admin-curated device manifests (staff-curated; read for context) |
| `get_manifest` | Fetch one manifest document |
| `update_manifest` | Edit a manifest (staff-controlled; external teams normally won't use this) |
...
Base64 inflates payloads ~33% and a single JSON-RPC body is capped at ~48 MB,
so large ZIPs (real language packs routinely exceed 100 MB) are sent in
chunks:

1. **`begin_upload`** — optional `filename`, and optional-but-recommended
   `totalSize` (decoded bytes; when provided, completeness is enforced before
   the file can be used). Returns an `uploadId`.
2. **`upload_chunk`** — `{ uploadId, chunkIndex, dataBase64 }`, called once
   per chunk, **strictly sequential** (`chunkIndex` 0, 1, 2, …). Recommended
   chunk size 4–8 MB of raw bytes; hard max 16 MB decoded per call. Each
...
- Max assembled size **200 MB** (same as REST).
- Sessions are **single-use** (consumed and deleted by the upload tool) and
  **private to the MCP token** that created them.
...
Despite its historical name, `upload_core_game` uploads the **engine tier**.
...
- `hasCoreLevel`: use `false` for Layout A (2-tier) and `true` for Layout B
  (3-tier). A Layout B engine's `urlTemplate` includes `{book}` (e.g.
  `…?cr_book={book}&cr_lang={lang}`).
...
```json
{
  ...
  "arguments": {
    ...
    "filename": "storyplayer-book-my-first-book.zip",
    "zipBase64": "<base64 of storyplayer-book-my-first-book.zip>"
  }
}
```
...
- `bookTitle` is optional; if omitted, an existing book keeps its current
  ...
  **existing** book requires an "edit"-scope token — with an upload-only
  token, upload new versions without `bookTitle` (or with the unchanged
  title).
- `versionNotes` (optional) works the same as on the other upload tools.

### 8.4a Step 4 — upload each language pack (`upload_language_pack`)

One call per language. For Layout B, call once per (core, language) and pass
the core-unit slug as `bookSlug` (the frozen wire name). For Layout A, omit
`bookSlug`.
...
and `upload_language_pack` — a free-text note stored with that uploaded
version (visible in the inventory and manifest tooling; editable later with
`update_version_notes`). Use it to describe what changed in the version.
...
2. Ask the Curious Learning team to preview the content in their admin CMS
   ...
   be listed with its `zips[]` (`core` + `lang-<code>` for Layout A; a Layout
   B tile also includes `book-<slug>`).
...
prior production version of the same logical item. A tile is eligible for
devices only when all of its Layout A tiers (engine + lang) have a production
version; a Layout B tile also requires its core tier in
production.

> **Promotion alone is not live.** Promotion makes your version the current
> one *in the CMS* — it does **not** put it on production devices. Delivery
> requires the staff Publish step below (§8.5a).

**Version retention.** Uploading a new version never deletes old ones — every
previously uploaded version stays stored and resolvable (the CMS can pin a
manifest entry to an older version). Reclaiming space is an explicit,
staff-only cleanup that removes only superseded *development* versions and
always skips any version pinned by a manifest — so you can safely iterate
with repeated uploads, and a pinned older version of your content will not
vanish out from under a device manifest.

### 8.5a Step 6 — delivery: the staff Publish (Curious Learning-only)

Production devices do not read the live CMS API. They download all runtime
content (manifests, per-file packs, icons) as **static artifacts from a
public CDN-fronted bucket**. The CMS regenerates its staging copy of those
artifacts automatically after every mutation (including your uploads and
promotions), but content only reaches devices when Curious Learning staff
press **Publish** in the admin CMS, which syncs the staging artifacts to the
public bucket behind the CDN.

Practical consequence for external teams: after your content is promoted,
**coordinate with the Curious Learning team to publish** — until they do,
installed apps will keep serving the previously published content, no matter
what `list_inventory` or the manifest endpoints show.

### 8.6 Alternatives: REST endpoints and the admin UI

All of the below require an allowlisted, authenticated session (Google
sign-in via the admin site) rather than an MCP token.
...
4. Read `cr_lang` and `cr_user_id` from `window.location.search`; Layout B games also read `cr_book`.
5. Per-language audio under `lang/<code>/audios/`; MP3/WAV; relative paths only, including inside JSON data.
6. Choose the packaging layout from the content model: Layout A uses `<engine>-core.zip` (engine tier, `index.html` at root, no `lang/`) + one `<engine>-lang-<code>.zip` per language. Layout B adds `<engine>-book-<coreSlug>.zip` for illustrations/structure or other content reused across languages, plus one language ZIP per (core, language). The engine remains the engine in both layouts.
...
- *1.4 — 2026-08-19 — Agent — Large ZIPs no longer need the REST fallback: new chunked-upload MCP tools (`begin_upload`, `upload_chunk`, `abort_upload`, all available to "upload only" tokens) stream files in sequential base64 chunks into a server-side session; `upload_core_game` / `upload_language_pack` accept `uploadId` (+ optional `sha256` integrity check) in place of `zipBase64`, up to the same 200 MB ceiling as REST. §8.1 tool table expanded to twelve tools, new §8.1a documents the chunked flow (sequential chunks, 16 MB max per chunk, single-use owner-scoped sessions, 60-minute inactivity expiry), the old "prefer REST above a few tens of MB" caveat removed, Appendix A step 10 updated.*
- *1.3 — 2026-08-19 — Agent — Retired the shared MCP API key: `/mcp` now authenticates with **personal MCP tokens** created by a CMS user on the admin "MCP tokens" tab (shown once at creation, copy immediately; deletable at any time for instant revocation). §8 prerequisites and §8.1 document the token workflow and the two permission scopes ("upload only" = uploads + read tools; "edit" adds promote/demote, version notes, manifest edits), the client-config example uses a personal token, and Appendix A step 10 updated accordingly. Header usage (Bearer / `x-api-key`) is unchanged.*
- *1.2 — 2026-08-18 — Agent — Synced with the current CMS: §8.1 tool list expanded to all nine MCP tools (adds `demote_content`, `update_version_notes`, `list_manifests`, `get_manifest`, `update_manifest`); §8.4 documents the optional `versionNotes` upload parameter and the human-curated language catalog (uploads with an unknown `langCode` are rejected — languages are never created implicitly); §8.5 adds the version-retention model (old versions kept; staff-only cleanup skips manifest-pinned versions) and corrects the delivery story — promotion makes a version current in the CMS but does not reach devices; new §8.5a documents the staff Publish step that syncs static artifacts to the public CDN-fronted bucket, the actual go-live; Appendix A steps updated accordingly.*
